...
export function matchesKitchenButtonLabel(value) {
  ...
  return catalog ? matchesCatalogButtonLabel(catalog, value) : false;
}

/** Page admin types catalog keyword in Inbox → send photos to customer. Default on. */
export function shouldSendCatalogOnAdminEcho() {
  if (process.env.ADMIN_CATALOG_ECHO_SEND === "false") return false;
  return true;
}
...
export function isKitchenButtonPostback(postback) {
  const catalog = findCatalogFromPostback(postback);
  return catalog?.id === CATALOG_IDS.KITCHEN;
}
...
export function isKitchenQuickReply(quickReply) {
  const catalog = findCatalogFromQuickReply(quickReply);
  return catalog?.id === CATALOG_IDS.KITCHEN;
}
...
export function getAdReferralSkipReason(referral) {
  ...
  if (!isAdFilterConfigured()) {
    return "ad filter off — photos send on Chat builder button tap only";
  }
