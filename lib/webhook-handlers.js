 ...
 * may fall outside the 24h customer window, so use the human-agent tag (7 days).
 * Customer-initiated triggers stay RESPONSE — the user just interacted.
...
function getMessagingTagForTrigger(trigger) {
  ...
  const override = process.env.ADMIN_ECHO_MESSAGE_TAG?.trim();
