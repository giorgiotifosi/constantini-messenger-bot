...
/**
 * Agent-initiated sends (operator typed a keyword / assigned a label in Inbox)
 * may fall outside the 24h customer window. The HUMAN_AGENT tag (7 days) covers
 * that, but it requires prior Meta approval — without it Meta rejects the send
 * (#100 / subcode 2018276). So default to no tag (RESPONSE, same as customers)
 * and only opt in via ADMIN_ECHO_MESSAGE_TAG once Human Agent is approved.
 * @type {Set<AlbumTrigger>}
 */
const HUMAN_AGENT_TRIGGERS = new Set(["admin_echo", "inbox_label"]);

/**
 * @param {AlbumTrigger} trigger
 * @returns {string | undefined}
 */
function getMessagingTagForTrigger(trigger) {
  if (!HUMAN_AGENT_TRIGGERS.has(trigger)) return undefined;
  return process.env.ADMIN_ECHO_MESSAGE_TAG?.trim() || undefined;
}
...
/**
 * Hands thread control back to the bot so photos can be sent in conversations
 * that originated from a comment (where Inbox holds control by default).
 * Silently ignored if the bot already owns the thread.
 * @param {string} psid
 */
async function takeThreadControl(psid) {
  ...
  if (!pageId) {
    console.warn("take_thread_control skipped: PAGE_ID is not set");
    return;
  }
  try {
    const res = await fetch(
      `https://graph.facebook.com/v18.0/${pageId}/take_thread_control?access_token=${encodeURIComponent(token)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ recipient: { id: psid }, metadata: "sending catalog" }),
      }
    );
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      console.warn(`take_thread_control failed for ${psid}: ${JSON.stringify(data.error ?? res.status)}`);
    } else {
      console.log(`take_thread_control ok for ${psid}`);
    }
  } catch (error) {
    console.warn(`take_thread_control error for ${psid}:`, error.message);
  }
}
...
async function sendAlbumOnce(item, batchSent) {
  ...
  await enqueueAlbumSendForPsid(item.psid, item.catalogId, async () => {
    ...
    if (item.trigger === "admin_echo") {
      await takeThreadControl(item.psid);
    }
    ...
    try {
      const result = await sendProductAlbumToUser(
        item.psid,
        item.catalogId,
        { trigger: item.trigger, tag: getMessagingTagForTrigger(item.trigger) }
      );
      console.log(
