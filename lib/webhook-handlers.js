...
function considerReferralChatOpen(referral, psid, trigger, pending) {
  ...
  if (!shouldSendPhotosOnChatOpen()) {
    console.log(
      `Chat open referral ignored for ${psid} (button-only; set MESSENGER_AD_CHAT_OPEN=true to enable)`
    );
    return;
  }
...
export function collectKitchenSendsFromMessagingEvent(event, pending) {
  ...
  if (event.postback) {
    ...
    console.log(
      `Postback ignored for ${psid}: title=${JSON.stringify(event.postback.title)} payload=${JSON.stringify(event.postback.payload)} — set unique KITCHEN_POSTBACK_PAYLOAD / SOFT_FURNITURE_POSTBACK_PAYLOAD in Chat builder + Vercel`
    );
  ...
  const quickReplyCatalog = message.quick_reply
    ? findCatalogFromQuickReply(message.quick_reply, text)
    : null;
