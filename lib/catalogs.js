/** @typedef {'kitchen' | 'soft_furniture' | 'bedroom'} CatalogId */
...
export function getProductCatalogs() {
  return [
    ...
    {
      id: CATALOG_IDS.BEDROOM,
      buttonLabel: process.env.BEDROOM_BUTTON_LABEL?.trim() || "საძინებელი",
      urlsEnv: "BEDROOM_IMAGE_URLS",
      urlPrefix: "BEDROOM_IMAGE_URL_",
      postbackPayloadEnv: "BEDROOM_POSTBACK_PAYLOAD",
    },
  ];
}
 ...
 * Uses the same matching as customer triggers (exact, soft-furniture markers,
 * and bounded fuzzy) but capped at a short length so long automated greeting
 * echoes never trigger a send.
 * @param {string | undefined} text
 * @returns {ProductCatalog | null}
 */
export function findCatalogFromAdminEcho(text) {
  ...
  return findCatalogByButtonValue(trimmed);
}
