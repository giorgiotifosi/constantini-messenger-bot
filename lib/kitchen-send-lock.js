...
/**
 * @typedef {{ allowRepeat?: boolean }} AlbumSendLockOptions
 */
 ...
 * @param {AlbumSendLockOptions} [options]
 * @returns {Promise<boolean>}
 */
export async function tryAcquireAlbumSend(
  psid,
  catalogId,
  batchSent,
  options = {}
  ...
  // Admin-triggered resends bypass the persistent dedupe entirely: the operator
  // explicitly retyped the keyword, so only guard against duplicates within the
  // same webhook batch (batchSent above).
 ...
 * @param {AlbumSendLockOptions} [options]
 */
export async function releaseAlbumSend(
  psid,
  catalogId,
  success,
  options = {}
) {
  // Admin resends never touched the persistent lock, so leave it untouched
  // (a concurrent customer send may legitimately hold it).
  if (options.allowRepeat) return;
