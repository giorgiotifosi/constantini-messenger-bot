...
export async function sendKitchenImages(recipientId, imageUrls) {
  ...
  // 1) Pre-upload to Meta, then one album (best for large sets)
  try {
  ...
  // 2) Direct URL album (works well for small sets)
  try {
...
export async function handleIncomingText(senderId, text) {
  ...
  const result = await sendKitchenImages(senderId, imageUrls);
