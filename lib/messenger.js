...
async function callSendApi(body) {
  ...
  if (!response.ok) {
    ...
    const detail = [
      ...
      .filter(Boolean)
      .join(" | ");
    throw new Error(`Send API error: ${detail || response.statusText}`);
  }
...
/**
 * Send images one-by-one (most compatible; works with WebP and older API behavior).
 * @param {string} recipientId
...
/**
 * Send up to 30 images in one Messenger message (album). May fail with some hosts/formats.
 * @param {string} recipientId
 * @param {string[]} imageUrls
 */
...
/**
 * Try album first; on Meta internal errors, fall back to sequential sends.
 * @param {string} recipientId
 * @param {string[]} imageUrls
 */
export async function sendKitchenImages(recipientId, imageUrls) {
  ...
  try {
    ...
    console.warn(
      "Album send failed, falling back to sequential images:",
      albumError.message
    );
...
export async function handleIncomingText(senderId, text) {
  ...
  const { mode } = await sendKitchenImages(senderId, imageUrls);

  return { handled: true, imageCount: imageUrls.length, mode };
}
