...
/**
 * Send images one-by-one (fallback only).
 * @param {string} recipientId
 * @param {string[]} imageUrls
 */
...
export async function sendKitchenImages(recipientId, imageUrls) {
  ...
  console.log(`Preparing ${count} kitchen images for ${recipientId}`);
  ...
  // 1) Direct URL album (fastest — best right after ad chat open)
  try {
  ...
  // 2) Pre-upload to Meta, then one album (more reliable for large sets)
  try {
  ...
  // 3) Last resort — separate messages (avoid for large albums)
  console.warn(
    `Falling back to sequential send for ${count} images — check image format (use JPG/PNG)`
  );
...
export async function sendKitchenAlbumOnChatOpen(psid) {
  console.log(`Chat open → sending kitchen album to ${psid}`);
