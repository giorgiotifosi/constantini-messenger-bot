...
/**
 * TEMPORARY — pages_messaging App Review demo subset. KITCHEN ONLY.
 *
 * https://constantini.ge/wp-content/uploads/2026/05/5.jpg is currently rejected
 * by the Meta Attachment Upload API:
 *
 *   HTTP 400 | code 100 | error_subcode 2018047
 *   "(#100) Upload attachment failure"
 *
 * Because an album may only be sent once every image has uploaded, that single
 * image aborts the whole 30-photo kitchen album. To record the App Review
 * screencast the kitchen catalog is temporarily limited to the first
 * DEMO_KITCHEN_IMAGE_LIMIT usable images, with the failing URL excluded
 * outright so it can never be picked up by the subset.
 *
 * The album architecture is untouched: these 4 URLs still go through
 * /message_attachments uploads → attachment_ids → one /messages album request.
 *
 * TO REVERT after App Review: delete DEMO_KITCHEN_IMAGE_LIMIT,
 * DEMO_BLOCKED_IMAGE_URLS and applyDemoKitchenSubset(), and drop the single
 * call to it in sendProductAlbumToUser(). Nothing else depends on them.
 */
const DEMO_KITCHEN_IMAGE_LIMIT = 30;
const DEMO_BLOCKED_IMAGE_URLS = new Set([
  "https://constantini.ge/wp-content/uploads/2026/05/5.jpg",
]);
...
async function uploadImageAttachment(imageUrl, index, total, catalogId, recipientId) {
  ...
  try {
    ...
    // Propagate untouched: uploadAllImages decides whether to skip this one
    // image or (if every image failed) abort the whole send.
    throw error;
  }
}

/**
 * Upload every image, preserving the original order among the ones that
 * succeed.
 *
 * Resilient, not fail-fast: every image is attempted regardless of whether
 * others fail. A failed upload is logged (by uploadImageAttachment, above)
 * and skipped rather than aborting the batch, so one bad image URL doesn't
 * take down the rest of the album.
 *
 * @param {string[]} imageUrls
 * @param {import("@/lib/catalogs").CatalogId} catalogId
 * @param {string} recipientId
 * @returns {Promise<{ attachmentIds: string[], failed: Array<{ index: number, url: string, error: unknown }> }>}
 */
async function uploadAllImages(imageUrls, catalogId, recipientId) {
  /** @type {Array<{ index: number, url: string, error: unknown }>} */
  const failed = [];
  ...
  // `results` is index-aligned with imageUrls; dropping the null (failed)
  // slots keeps the remaining attachment_ids in their original image order.
  const attachmentIds = results.filter((id) => id !== null);

  return { attachmentIds, failed };
}
 ...
 * every attachment ID that uploaded successfully.
 *
 * A failed upload no longer aborts the whole send — it's logged and skipped
 * so the rest of the album still goes out. Only when *every* image fails is
 * the album request skipped entirely (nothing usable to send).
 *
 * @param {string} recipientId
 * @param {string[]} imageUrls
 * @param {import("@/lib/catalogs").CatalogId} catalogId
...
export async function sendCatalogAlbum(recipientId, imageUrls, catalogId) {
  ...
  // Step 1 — upload every image. Failed uploads are skipped, not fatal,
  // unless every single one fails (checked below).
  const { attachmentIds, failed } = await uploadAllImages(
    imageUrls,
    catalogId,
    recipientId
  );
  ...
  if (failed.length > 0) {
    console.warn(
      `Album uploads partially failed — skipping failed image(s) ${JSON.stringify(
        {
          catalog: catalogId,
          recipient_psid: recipientId,
          image_count: count,
          succeeded_count: attachmentIds.length,
          failed_count: failed.length,
          skipped: failed.map(({ index, url, error }) => ({
            image_index: index + 1,
            image_url: url,
            ...describeGraphError(error),
          })),
  ...
  if (attachmentIds.length === 0) {
    console.error(
      `Album aborted: no usable attachments ${JSON.stringify({
        catalog: catalogId,
        recipient_psid: recipientId,
        image_count: count,
        failed_count: failed.length,
    ...
    throw new Error(
      `All ${count} image upload(s) failed for catalog "${catalogId}" — no usable attachments to send`
    );
  }
  ...
  // Step 2 — one album request with every successfully uploaded
  // attachment_id, in image order.
  try {
...
/**
 * TEMPORARY — see DEMO_KITCHEN_IMAGE_LIMIT above. Kitchen only; every other
 * catalog is returned untouched.
 * @param {import("@/lib/catalogs").CatalogId} catalogId
 * @param {string[]} imageUrls
 * @returns {string[]}
 */
function applyDemoKitchenSubset(catalogId, imageUrls) {
  if (catalogId !== CATALOG_IDS.KITCHEN) return imageUrls;

  const blocked = imageUrls.filter((url) => DEMO_BLOCKED_IMAGE_URLS.has(url));
  const usable = imageUrls.filter((url) => !DEMO_BLOCKED_IMAGE_URLS.has(url));
  const subset = usable.slice(0, DEMO_KITCHEN_IMAGE_LIMIT);

  console.warn(
    `TEMP App Review demo subset ${JSON.stringify({
      catalog: catalogId,
      configured_images: imageUrls.length,
      demo_images: subset.length,
      excluded_known_failing: blocked,
      urls: subset,
      reason: "image 5 fails Meta Attachment Upload API (400 / code 100 / subcode 2018047)",
    })}`
  );
...
export async function sendProductAlbumToUser(recipientId, catalogId, options = {}) {
  ...
  const imageUrls = applyDemoKitchenSubset(
    catalogId,
    getImageUrlsForCatalog(catalog)
  );
