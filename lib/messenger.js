...
export async function sendKitchenImages(recipientId, imageUrls, catalogId, sendOptions = {}) {
  ...
  console.log(
    `Preparing ${count} images for ${recipientId} catalog=${catalogId} (mode=${mode}, tag=${sendOptions.tag ?? "RESPONSE"})`
  );

  if (count === 1) {
    await sendSingleImage(recipientId, imageUrls[0], sendOptions);
    return { mode: "single", imageCount: count };
  }

  if (mode === "sequential") {
    await sendImagesSequential(recipientId, imageUrls, sendOptions);
    return { mode: "sequential", imageCount: count };
  }

  try {
    await sendImageAlbum(recipientId, imageUrls, sendOptions);
  ...
  try {
    ...
    await sendImageAlbumByIds(recipientId, attachmentIds, sendOptions);
